...
function quotePathIfNeeded(path21) {
  if (/\s/.test(path21))
    return `"${path21}"`;
  return path21;
}
...
var InternalReporter = class {
  ...
  onConfigure(config2) {
    ...
    this._monotonicStartTime = monotonicTime2();
  ...
  async onEnd(result) {
    ...
    return await this._reporter.onEnd?.({
      ...
      duration: monotonicTime2() - this._monotonicStartTime
    });
  }
  async onExit() {
    await this._reporter.onExit?.();
...
var { monotonicTime: monotonicTime3, timeOrigin } = require("playwright-core/lib/coreBundle").iso;
var { raceAgainstDeadline: raceAgainstDeadline2 } = require("playwright-core/lib/coreBundle").iso;
var { killProcessTree } = require("playwright-core/lib/coreBundle").utils;
var ProcessHost = class extends import_events.EventEmitter {
  ...
  async stop() {
    ...
    let lastHeartbeat = monotonicTime3();
    const onHeartbeat = () => lastHeartbeat = monotonicTime3();
    ...
    try {
      while (true) {
        ...
        if (!result.timedOut)
          return;
        if (monotonicTime3() < lastHeartbeat + timeout)
          continue;
...
function filterProjects(projects, projectNames, includeNonDefault) {
  if (!projectNames)
...
// packages/playwright/src/reporters/coverage.ts
var import_fs5 = __toESM(require("fs"));
var import_path7 = __toESM(require("path"));
var import_util7 = require("../util");
var { mergeIstanbulCoverage, sortedIstanbulCoverage } = require("playwright-core/lib/coreBundle").iso;
var { ZipFile } = require("playwright-core/lib/coreBundle").utils;
var CoverageReporter = class {
  constructor(options) {
    this._traceFiles = /* @__PURE__ */ new Set();
    this._options = options;
  }
  version() {
    return "v2";
...
No code coverage was collected.`,
    ...
    const outputDir = (0, import_util7.resolveReporterOutputPath)("coverage", this._options.configDir, this._options.outputDir);
    await import_fs5.default.promises.mkdir(outputDir, { recursive: true });
    const mergedJson = sortedIstanbulCoverage(coverage);
    await import_fs5.default.promises.writeFile(import_path7.default.join(outputDir, "coverage-final.json"), JSON.stringify(mergedJson));
    await import_fs5.default.promises.writeFile(import_path7.default.join(outputDir, "lcov.info"), lcovReport(mergedJson));
    ...
    const lines = [
      ...
      `Coverage report written to ${import_path7.default.relative(process.cwd(), outputDir)}`
    ];
    if (!hasHtml)
      lines.push(`For an HTML report, install istanbul-lib-coverage, istanbul-lib-report and istanbul-reports, or run 'npx nyc report --temp-dir=${import_path7.default.relative(process.cwd(), outputDir)} --reporter=html'.`);
    writeLine(lines.join("\n"));
  }
  // Istanbul report libraries are not shipped with Playwright, use the project's if any.
  _tryWriteHtmlReport(mergedJson, outputDir) {
    try {
      const resolveFrom = (name) => require.resolve(name, { paths: [this._options.configDir] });
...
function computeSummary(mergedJson) {
...
function countHits(metric, counts) {
...
function lcovReport(mergedJson) {
  const out = [];
...
var coverage_default = CoverageReporter;
...
var GitHubLogger = class {
  ...
  _log(message, type = "notice", options = {}) {
    ...
    process.stdout.write((0, import_util8.stripAnsiEscapes)(`::${type} ${configs}::${message}
`));
  }
...
var GitHubReporter = class extends TerminalReporter {
  ...
  _printSlowTestAnnotations() {
    this.getSlowTests().forEach(([file, duration]) => {
      const filePath = workspaceRelativePath(import_path8.default.join(process.cwd(), file));
...
function workspaceRelativePath(filePath) {
  return import_path8.default.relative(process.env["GITHUB_WORKSPACE"] ?? "", filePath);
}
  ...
  onBegin(suite) {
    ...
    for (const project of this.config.projects) {
      if (this._isSubdirectory(outputFolder, project.outputDir) || this._isSubdirectory(project.outputDir, outputFolder)) {
        ...
        writeLine2(colors2.red(`Configuration Error: HTML reporter output folder clashes with the tests output folder:`));
        writeLine2(`
    html reporter folder: ${colors2.bold(outputFolder)}
    test results folder: ${colors2.bold(project.outputDir)}`);
        writeLine2("");
        writeLine2(`HTML reporter will clear its output directory prior to being generated, which will lead to the artifact loss.
`);
      }
    }
    this.suite = suite;
  ...
  _resolveOptions() {
    const outputFolder = reportFolderFromEnv() ?? (0, import_util9.resolveReporterOutputPath)("playwright-report", this._options.configDir, this._options.outputFolder);
    return {
      outputFolder,
  ...
  _isSubdirectory(parentDir, dir) {
    const relativePath = import_path9.default.relative(parentDir, dir);
    return !!relativePath && !relativePath.startsWith("..") && !import_path9.default.isAbsolute(relativePath);
  }
  onError(error) {
  ...
  async onExit() {
    ...
    if (shouldOpen) {
      ...
      const relativeReportPath = this._outputFolder === standaloneDefaultFolder() ? "" : " " + import_path9.default.relative(process.cwd(), this._outputFolder);
      const hostArg = this._host ? ` --host ${this._host}` : "";
      const portArg = this._port ? ` --port ${this._port}` : "";
      writeLine2("");
      writeLine2("To open last HTML report run:");
      writeLine2(colors2.cyan(`
...
function reportFolderFromEnv() {
  ...
  return envValue ? import_path9.default.resolve(envValue) : void 0;
}
function getHtmlReportOptionProcessEnv() {
  ...
  if (!isHtmlReportOption(htmlOpenEnv)) {
    writeLine2(colors2.red(`Configuration Error: HTML reporter Invalid value for PLAYWRIGHT_HTML_OPEN: ${htmlOpenEnv}. Valid values are: ${htmlReportOptions.join(", ")}`));
    return void 0;
  }
  return htmlOpenEnv;
}
...
function standaloneDefaultFolder() {
  return reportFolderFromEnv() ?? (0, import_util9.resolveReporterOutputPath)("playwright-report", process.cwd(), void 0);
}
async function resolveReportFolder(reportPath) {
  const stat = await import_fs6.default.promises.stat(reportPath).catch(() => null);
  if (!stat)
...
async function extractReportZip(zipPath) {
  const tempDir = await import_fs6.default.promises.mkdtemp(import_path9.default.join(import_os.default.tmpdir(), "playwright-show-report-"));
  const cleanup = () => {
    try {
      import_fs6.default.rmSync(tempDir, { recursive: true, force: true });
    } catch {
    }
  };
  ...
  const hasIndex = await import_fs6.default.promises.access(import_path9.default.join(tempDir, "index.html")).then(() => true, () => false);
  if (!hasIndex) {
    cleanup();
...
async function showHTMLReport(reportFolder, host = "localhost", port, testId) {
  ...
  try {
    ...
    writeLine2(colors2.red(e.message));
  ...
  writeLine2("");
  writeLine2(colors2.cyan(`  Serving HTML report at ${url}. Press Ctrl+C to quit.`));
  if (testId)
    url += `#?testId=${testId}`;
...
var HtmlBuilder = class {
  constructor(yazl3, config2, outputDir, attachmentsBaseURL, doNotInlineAssets, options) {
    ...
    import_fs6.default.mkdirSync(this._reportFolder, { recursive: true });
    this._attachmentsBaseURL = attachmentsBaseURL;
  }
  async build(metadata, projectSuites, result, topLevelErrors, machines) {
    ...
    if (this._hasTraces) {
      const traceViewerFolder = import_path9.default.join(require.resolve("playwright-core"), "..", "lib", "vite", "traceViewer");
      const traceViewerTargetFolder = import_path9.default.join(this._reportFolder, "trace");
      const traceViewerAssetsTargetFolder = import_path9.default.join(traceViewerTargetFolder, "assets");
      import_fs6.default.mkdirSync(traceViewerAssetsTargetFolder, { recursive: true });
      for (const file of import_fs6.default.readdirSync(traceViewerFolder)) {
        if (file.endsWith(".map") || file.includes("watch") || file.includes("assets"))
          continue;
        await copyFileAndMakeWritable(import_path9.default.join(traceViewerFolder, file), import_path9.default.join(traceViewerTargetFolder, file));
      }
      for (const file of import_fs6.default.readdirSync(import_path9.default.join(traceViewerFolder, "assets"))) {
        if (file.endsWith(".map") || file.includes("xtermModule"))
          continue;
        await copyFileAndMakeWritable(import_path9.default.join(traceViewerFolder, "assets", file), import_path9.default.join(traceViewerAssetsTargetFolder, file));
      }
    }
  ...
  async _writeStaticAssets() {
    const appFolder = import_path9.default.join(require.resolve("playwright-core"), "..", "lib", "vite", "htmlReport");
    const reportIndexFile = import_path9.default.join(this._reportFolder, "index.html");
    if (this._doNotInlineAssets) {
      const html = await import_fs6.default.promises.readFile(import_path9.default.join(appFolder, "index.html"), "utf-8");
      await Promise.all([
        import_fs6.default.promises.writeFile(reportIndexFile, html),
        import_fs6.default.promises.copyFile(import_path9.default.join(appFolder, "report.js"), import_path9.default.join(this._reportFolder, "report.js")),
        import_fs6.default.promises.copyFile(import_path9.default.join(appFolder, "report.css"), import_path9.default.join(this._reportFolder, "report.css"))
      ]);
    } else {
      let html = await import_fs6.default.promises.readFile(import_path9.default.join(appFolder, "index.html"), "utf-8");
      const [js, css] = await Promise.all([
        import_fs6.default.promises.readFile(import_path9.default.join(appFolder, "report.js"), "utf-8"),
        import_fs6.default.promises.readFile(import_path9.default.join(appFolder, "report.css"), "utf-8")
      ]);
      ...
      await import_fs6.default.promises.writeFile(reportIndexFile, html);
    }
    return reportIndexFile;
  }
  async _writeReportData(filePath) {
    import_fs6.default.appendFileSync(filePath, '<template id="playwrightReportBase64">data:application/zip;base64,');
    await new Promise((resolve, reject) => {
      this._dataZipFile.end(void 0, () => {
        this._dataZipFile.outputStream.pipe(new Base64Encoder()).pipe(import_fs6.default.createWriteStream(filePath, { flags: "a" })).on("close", resolve).on("error", reject);
      });
    });
    import_fs6.default.appendFileSync(filePath, "</template>");
  }
  ...
  _processSuite(suite, projectName, path21, deep, outTests) {
    const newPath = [...path21, suite.title];
  ...
  _createTestEntry(test, projectName, path21) {
    const duration = test.results.reduce((a, r) => a + r.duration, 0);
    const location = this._relativeLocation(test.location);
    path21 = path21.slice(1).filter((path22) => path22.length > 0);
    ...
    return attachments.map((a) => {
      ...
      if ((a.name === "stdout" || a.name === "stderr") && a.contentType === "text/plain") {
        if (lastAttachment && lastAttachment.name === a.name && lastAttachment.contentType === a.contentType) {
          lastAttachment.body += (0, import_util9.stripAnsiEscapes)(a.body);
          return null;
        }
        a.body = (0, import_util9.stripAnsiEscapes)(a.body);
        lastAttachment = a;
        return a;
      }
      if (a.path) {
        let fileName = a.path;
        try {
          const buffer = import_fs6.default.readFileSync(a.path);
          const sha1 = calculateSha12(buffer) + import_path9.default.extname(a.path);
          fileName = this._attachmentsBaseURL + sha1;
          import_fs6.default.mkdirSync(import_path9.default.join(this._reportFolder, "data"), { recursive: true });
          import_fs6.default.writeFileSync(import_path9.default.join(this._reportFolder, "data", sha1), buffer);
        } catch (e) {
        }
        return {
          name: a.name,
      ...
      if (a.body instanceof Buffer) {
        ...
        import_fs6.default.mkdirSync(import_path9.default.join(this._reportFolder, "data"), { recursive: true });
        const extension = sanitizeForFilePath2(import_path9.default.extname(a.name).replace(/^\./, "")) || mime2.getExtension(a.contentType) || "dat";
        const sha1 = calculateSha12(a.body) + "." + extension;
        import_fs6.default.writeFileSync(import_path9.default.join(this._reportFolder, "data", sha1), a.body);
        return {
          name: a.name,
          contentType: a.contentType,
  ...
  _relativeLocation(location) {
    if (!location)
      return void 0;
    const file = toPosixPath2(import_path9.default.relative(this._config.rootDir, location.file));
    return {
      file,
      line: location.line,
...
function createSnippets(stepsInFile) {
  for (const file of stepsInFile.keys()) {
    let source;
    try {
      source = import_fs6.default.readFileSync(file, "utf-8") + "\n//";
    } catch (e) {
      continue;
    }
...
function createErrorCodeframe(message, location) {
  let source;
  try {
    source = import_fs6.default.readFileSync(location.file, "utf-8") + "\n//";
  } catch (e) {
    return;
  }
  return babel2.codeFrameColumns(
    ...
    },
    {
      ...
      linesAbove: 100,
      linesBelow: 100,
      message: (0, import_util9.stripAnsiEscapes)(message).split("\n")[0] || void 0
    }
  );
}
...
var JSONReporter = class {
  ...
  _relativeLocation(location) {
    ...
    return {
      file: toPosixPath3(import_path10.default.relative(this.config.rootDir, location.file)),
      line: location.line,
      column: location.column
    };
  }
  ...
  _mergeTestsFromSuite(to, from) {
    ...
    for (const spec of from.specs || []) {
      const toSpec = to.specs.find((s) => s.title === spec.title && s.file === toPosixPath3(import_path10.default.relative(this.config.rootDir, spec.file)) && s.line === spec.line && s.column === spec.column);
      if (toSpec)
...
async function outputReport(report, resolvedOutputFile) {
  ...
  if (resolvedOutputFile) {
    await import_fs7.default.promises.mkdir(import_path10.default.dirname(resolvedOutputFile), { recursive: true });
    await import_fs7.default.promises.writeFile(resolvedOutputFile, reportString);
  } else {
...
var JUnitReporter = class {
  ...
  async onEnd(result) {
    ...
    if (this.resolvedOutputFile) {
      await import_fs8.default.promises.mkdir(import_path11.default.dirname(this.resolvedOutputFile), { recursive: true });
      await import_fs8.default.promises.writeFile(this.resolvedOutputFile, reportString);
    } else {
      console.log(reportString);
    }
  }
  ...
  _addFailureEntry(test, errorInfo, entry) {
    if (errorInfo) {
      entry.children.push({
        ...
        text: (0, import_util10.stripAnsiEscapes)(formatFailure(nonTerminalScreen, this.config, test, void 0, { omitTags: this.omitTags }))
      });
    ...
    entry.children.push({
      name: "failure",
      attributes: {
        message: `${import_path11.default.basename(test.location.file)}:${test.location.line}:${test.location.column} ${test.title}`,
        type: "FAILURE"
      },
      text: (0, import_util10.stripAnsiEscapes)(formatFailure(nonTerminalScreen, this.config, test, void 0, { omitTags: this.omitTags }))
    });
  ...
  async _appendStdIO(entry, results) {
    ...
    for (const result of results) {
      ...
      for (const attachment of result.attachments) {
        if (!attachment.path)
          continue;
        let attachmentPath = import_path11.default.relative(this.configDir, attachment.path);
        try {
          if (this.resolvedOutputFile)
            attachmentPath = import_path11.default.relative(import_path11.default.dirname(this.resolvedOutputFile), attachment.path);
        } catch {
          systemOut.push(`
Warning: Unable to make attachment path ${attachment.path} relative to report output file ${this.resolvedOutputFile}`);
        }
        try {
          await import_fs8.default.promises.access(attachment.path);
          systemOut.push(`
...
Warning: attachment ${attachmentPath} is missing`);
  ...
  async _buildRetryEntry(result, prefix) {
    ...
    entry.children.push({ name: "stackTrace", text: (0, import_util10.stripAnsiEscapes)(stackTrace) });
...
function classifyResultError(result) {
  ...
  if (!error)
    return null;
  const rawMessage = (0, import_util10.stripAnsiEscapes)(error.message || error.value || "");
...
function escape(text, stripANSIControlSequences, isCharacterData) {
  if (stripANSIControlSequences)
    text = (0, import_util10.stripAnsiEscapes)(text);
  if (isCharacterData) {
...
var ListReporter = class extends TerminalReporter {
  constructor(options) {
    const printFailuresInline = getAsBooleanFromENV5("PLAYWRIGHT_LIST_PRINT_FAILURES_INLINE", options?.printFailuresInline);
    super({ ...options, omitTags: getAsBooleanFromENV5("PLAYWRIGHT_LIST_OMIT_TAGS", options?.omitTags), lastResult: printFailuresInline });
    this._lastRow = 0;
    this._lastColumn = 0;
    ...
    this._failureIndex = /* @__PURE__ */ new Map();
  ...
  _testPrefix(index, statusMark) {
    const statusMarkLength = (0, import_util11.stripAnsiEscapes)(statusMark).length;
...
var ListModeReporter = class {
  ...
  onBegin(suite) {
    ...
    for (const test of tests) {
      const [, projectName, , ...titles] = test.titlePath();
      const location = `${import_path12.default.relative(this.config.rootDir, test.location.file)}:${test.location.line}:${test.location.column}`;
...
var PerfettoReporter = class {
  ...
  _formatError(error) {
    return (0, import_util12.stripAnsiEscapes)(formatError(nonTerminalScreen, error).message);
  }
  _relativePath(file) {
    return toPosixPath4(import_path13.default.relative(this._config.rootDir, file));
  }
  async _writeReport(result) {
    ...
    await import_fs9.default.promises.mkdir(import_path13.default.dirname(this._resolvedOutputFile), { recursive: true });
...
var ChunkWriter = class {
  constructor(file) {
    const fileStream = import_fs9.default.createWriteStream(file);
...
function computeCommandHash(config2, runOptions) {
  ...
  if (runOptions?.testList)
    command.testList = calculateSha13(import_fs10.default.readFileSync(runOptions.testList));
  if (runOptions?.testListInvert)
    command.testListInvert = calculateSha13(import_fs10.default.readFileSync(runOptions.testListInvert));
...
async function applySuggestedRebaselines(config2, reporter, filteredProjects) {
  ...
  const patchFile = import_path14.default.join(project.project.outputDir, "rebaselines.patch");
  for (const fileName of [...suggestedRebaselines.keys()].sort()) {
    const source = await import_fs11.default.promises.readFile(fileName, "utf8");
    const lines = source.split("\n");
    ...
    babel3.traverse(fileNode, {
      CallExpression: (path21) => {
        const node = path21.node;
    ...
    const relativeName = import_path14.default.relative(process.cwd(), fileName);
    files.push(relativeName);
    if (config2.config.updateSourceMethod === "overwrite") {
      await import_fs11.default.promises.writeFile(fileName, result);
    } else if (config2.config.updateSourceMethod === "3way") {
      await import_fs11.default.promises.writeFile(fileName, applyPatchWithConflictMarkers(source, result));
    } else {
      const gitFolder = findGitRoot(import_path14.default.dirname(fileName), gitCache);
      const relativeToGit = import_path14.default.relative(gitFolder || process.cwd(), fileName);
...
`);
  if (config2.config.updateSourceMethod === "patch") {
    await import_fs11.default.promises.mkdir(import_path14.default.dirname(patchFile), { recursive: true });
    await import_fs11.default.promises.writeFile(patchFile, patches.join("\n"));
    reporter.onStdErr(`
  ` + colors3.cyan("git apply " + import_path14.default.relative(process.cwd(), patchFile)) + "\n");
  }
}
...
function findGitRoot(dir, cache) {
  ...
  if (result !== void 0)
    return result;
  const gitPath = import_path14.default.join(dir, ".git");
  if (import_fs11.default.existsSync(gitPath) && import_fs11.default.lstatSync(gitPath).isDirectory()) {
    cache.set(dir, dir);
    return dir;
  }
  const parentDir = import_path14.default.dirname(dir);
  if (dir === parentDir) {
    cache.set(dir, null);
    return null;
  }
...
var WorkerHost = class extends ProcessHost {
  constructor(testGroup, options) {
    ...
    this._params = {
      ...
      artifactsDir: import_path15.default.join(options.outputDir, artifactsFolderName(workerIndex)),
      pauseOnError: options.pauseOnError,
      pauseAtEnd: options.pauseAtEnd
    };
  }
  async start() {
    await import_fs12.default.promises.mkdir(this._params.artifactsDir, { recursive: true });
  ...
  _onTestPaused(worker, params) {
    ...
    const sendMessage = async (message) => {
      try {
        ...
        const error = (0, import_util13.serializeError)(e);
    ...
    const taskLoop = async () => {
      for (const task of this._tasks) {
        ...
        try {
          ...
          errors.push((0, import_util14.serializeError)(e));
        } finally {
...
async function detectChangedTestFiles(baseCommit, configDir) {
  ...
  const untrackedFiles = gitFileList(["ls-files", "--others", "--exclude-standard"]).map((file) => import_path16.default.join(configDir, file));
  const [gitRoot] = gitFileList(["rev-parse", "--show-toplevel"]);
  const trackedFilesWithChanges = gitFileList(["diff", baseCommit, "--name-only"]).map((file) => import_path16.default.join(gitRoot, file));
...
var readDirAsync2 = (0, import_util15.promisify)(import_fs13.default.readdir);
var TestRun = class {
  constructor(config2, reporter, options) {
    ...
    this.filteredProjects = filterProjects(config2.projects, this.options.projectFilter, this.options.includeNonDefaultProjects);
    config2.config.filteredProjects = this.filteredProjects.map((p) => p.project);
  }
  onTestPaused(params) {
...
function createClearCacheTask(config2) {
  return {
    title: "clear cache",
    setup: async () => {
      await (0, import_util16.removeDirAndLogToConsole)(import_common9.cc.cacheDir);
    }
  };
}
    ...
    setup: async (testRun) => {
      ...
      await Promise.all(Array.from(outputDirs).map((outputDir) => removeFolders4([outputDir]).then(async ([error]) => {
        ...
        if (error.code === "EBUSY") {
          ...
          await Promise.all(entries.map((entry) => removeFolders4([import_path17.default.join(outputDir, entry)])));
        } else {
          throw error;
        }
      })));
    ...
    setup: async (testRun, errors) => {
      ...
      for (const [project, files] of testRun.projectFiles) {
        ...
        const suites = files.map((file) => {
          const title = import_path17.default.relative(testRun.config.config.rootDir, file);
  ...
  return {
    ...
    setup: async (testRun, errors, softErrors) => {
      ...
      if (testRun.options.grep || testRun.options.grepInvert) {
        const grepMatcher = testRun.options.grep ? (0, import_util16.createTitleMatcher)((0, import_util16.forceRegExp)(testRun.options.grep)) : () => true;
        const grepInvertMatcher = testRun.options.grepInvert ? (0, import_util16.createTitleMatcher)((0, import_util16.forceRegExp)(testRun.options.grepInvert)) : () => false;
...
var LastRunReporter = class {
  constructor(filteredProjects, listMode, lastFailedFileOverride) {
    ...
    if (override) {
      this._lastRunFile = import_path18.default.resolve(process.cwd(), override);
    } else {
      const [project] = filteredProjects;
      if (project)
        this._lastRunFile = import_path18.default.join(project.project.outputDir, ".last-run.json");
    }
  }
  async filterLastFailed() {
    if (!this._lastRunFile)
      return void 0;
    try {
      const lastRunInfo = JSON.parse(await import_fs14.default.promises.readFile(this._lastRunFile, "utf8"));
  ...
  async onEnd(result) {
    ...
    await import_fs14.default.promises.mkdir(import_path18.default.dirname(this._lastRunFile), { recursive: true });
    await import_fs14.default.promises.writeFile(this._lastRunFile, JSON.stringify(lastRunInfo, void 0, 2));
  }
};
...
var TestRunner = class extends import_events2.default {
  ...
  async listFiles(userReporter, projects) {
    ...
    if (!config2)
      return { status: "failed" };
    const options = { projectFilter: projects?.length ? projects : void 0, includeNonDefaultProjects: true };
  ...
  async _loadConfig(overrides) {
    try {
      ...
      return { config: null, error: (0, import_util17.serializeError)(e) };
    }
  }
  lastLoadedConfig() {
...
var import_util18 = __toESM(require("util"));
  ...
  _setInterceptStdio(interceptStdio) {
    ...
    if (interceptStdio) {
      if (debug6.log === originalDebugLog) {
        debug6.log = (...args) => {
          const string = import_util18.default.format(...args) + "\n";
...
async function runWatchModeLoop(configLocation, initialOptions) {
  ...
  transport.emit("open");
  const teleSuiteUpdater = new TeleSuiteUpdater({ pathSeparator: import_path19.default.sep, onUpdate() {
  } });
...
`);
  for (const file of dirtyTestFiles)
    process.stdout.write(` \xB7 ${import_path19.default.relative(rootDir, file)}
`);
  process.stdout.write(`
...
var { ZipFile: ZipFile2 } = require("playwright-core/lib/coreBundle").utils;
async function createMergedReport(config2, dir, reporterDescriptions, rootDirOverride) {
  ...
  const pathSeparator = rootDirOverride ? import_path20.default.sep : eventData.pathSeparatorFromMetadata ?? import_path20.default.sep;
  const pathPackage = pathSeparator === "/" ? import_path20.default.posix : import_path20.default.win32;
  ...
  for (const { reportFile, zipFile, eventPatchers, metadata, config: config3, fullResult } of eventData.reports) {
    ...
    const reportJsonl = await import_fs15.default.promises.readFile(reportFile);
...
async function extractAndParseReports(dir, shardFiles, internalizer, printStatus) {
  const shardEvents = [];
  await import_fs15.default.promises.mkdir(import_path20.default.join(dir, "resources"), { recursive: true });
  const reportNames = new UniqueFileNameGenerator();
  for (const file of shardFiles) {
    const absolutePath = import_path20.default.join(dir, file);
    printStatus(`extracting: ${(0, import_util19.relativeFilePath)(absolutePath)}`);
    const zipFile = new ZipFile2(absolutePath);
    const entryNames = await zipFile.entries();
    for (const entryName of entryNames.sort()) {
      let reportFile = import_path20.default.join(dir, entryName);
      ...
      await import_fs15.default.promises.writeFile(reportFile, content);
    }
    zipFile.close();
  }
  return shardEvents;
...
async function sortedShardFiles(dir) {
  const files = await import_fs15.default.promises.readdir(dir);
...
var UniqueFileNameGenerator = class {
  ...
  makeUnique(name) {
    ...
    const extension = import_path20.default.extname(name);
...
var AttachmentPathPatcher = class {
  ...
  _patchAttachments(attachments) {
    const resourceRoot = import_path20.default.resolve(this._resourceDir);
    for (const attachment of attachments) {
      if (!attachment.path)
        continue;
      const joined = import_path20.default.resolve(resourceRoot, attachment.path);
...
var PathSeparatorPatcher = class {
  constructor(from) {
    this._from = from ?? (import_path20.default.sep === "/" ? "\\" : "/");
    this._to = import_path20.default.sep;
  }
