...
var Fixture = class {
  constructor(runner, registration) {
    ...
    this._stepInfo = { title: `Fixture ${escapeWithQuotes(title, '"')}`, category: "fixture", stack: location ? [location] : void 0 };
...
var TestTracing = class {
  ...
  async startIfNeeded(value) {
    const defaultTraceOptions = { screenshots: true, snapshots: true, coverage: false, sources: true, attachments: true, live: false, mode: "off" };
    if (!value) {
...
var { createCallIdGenerator } = require("playwright-core/lib/coreBundle").iso;
...
var nextStepId = createCallIdGenerator();
var TestInfoImpl = class {
  ...
  _addStep(data, parentStep) {
    ...
    stack ??= filteredStackTrace3(captureRawStack());
    const step = {
      ...data,
      stepId,
...
var WorkerMain = class extends import_common3.ProcessRunner {
  ...
  async _runTest(test, retry, nextTest) {
    ...
    (0, import_expect.setExpectConfig)({ testInfo: null, filteredStackTrace: filteredStackTrace4, ignoreSnapshots: false, updateSnapshots: "default" });
    this.dispatchEvent("testEnd", buildTestEndPayload(testInfo));
